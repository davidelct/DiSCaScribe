...
// The waveform resolves left-to-right once the peaks are decoded, rather than
// appearing fully drawn. REVEAL_SPAN is the slice of that timeline any one bar
// spends growing — below 1 the bars overlap, so it reads as a sweep across the
// recording instead of a single bar marching along.
const REVEAL_DURATION_MS = 620
const REVEAL_SPAN = 0.45
...
export function AudioPlayer({ audioKey, className, placeholder = false }: AudioPlayerProps) {
  ...
  // 0 → 1 while the decoded waveform sweeps in; 1 once it has settled.
  ...
  const draw = useCallback(() => {
    ...
    // getComputedStyle forces a style recalc, and draw() runs every frame while
    // playing and through the reveal sweep. Resolve the colour once and refresh
    // it only when the canvas resizes (which is also when the theme can have
    // changed under it).
    if (colorRef.current === null) colorRef.current = getComputedStyle(canvas).color || "#3d6fa6"
    ...
    for (let i = 0; i < barCount; i += 1) {
      ...
      // Each bar starts growing when the sweep reaches it and rises from the
      // flat resting line to its real amplitude.
      const barStart = (i / barCount) * (1 - REVEAL_SPAN)
      const progress = easeOutCubic(Math.min(1, Math.max(0, (reveal - barStart) / REVEAL_SPAN)))
      const targetH = Math.max(minH, amp * maxH)
      const barH = minH + (targetH - minH) * progress
      const x = i * (barW + BAR_GAP)
      const played = (i + 0.5) / barCount <= frac
      ctx.globalAlpha = (played ? 0.95 : 0.28) * (0.35 + 0.65 * progress)
  ...
  /** Sweep the decoded waveform in, once, from the flat resting line. */
  const revealWaveform = useCallback(() => {
    ...
    // Park at the resting line, then let the thread settle before starting the
    // clock, so the sweep doesn't burn its first frames on a busy tick.
    revealRef.current = 0
    draw()
    ...
    revealRafRef.current = requestAnimationFrame(() => {
      ...
      const step = () => {
        ...
        revealRef.current = Math.min(1, elapsed / REVEAL_DURATION_MS)
        draw()
        if (revealRef.current < 1) revealRafRef.current = requestAnimationFrame(step)
      }
  ...
  useEffect(() => {
    ...
    // Draw the flat resting line at full presence; the sweep starts when the
    // peaks land, not while we're still fetching.
    revealRef.current = 1
  ...
  return (
    // Fades in over the loading strip it replaces, so the controls settle in
    // place rather than swapping hard once the recording is ready.
    <div className={cn("animate-rise flex items-center gap-3", className)}>
      <audio
        ref={audioRef}
        src={url ?? undefined}
        preload="metadata"
