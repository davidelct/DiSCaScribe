...
export async function createClinicalNoteText(params: ClinicalNoteRequest): Promise<string> {
  ...
  try {
    ...
    // The model writes the note itself, so bring the response back to the
    // canonical shape: unwrap any code fence, drop a stray document title, and
    // normalize heading variants (e.g. "## S") to the four SOAP headings the
    // section renderer expects.
    const normalizedMarkdown = normalizeMarkdownSections(
      stripDocumentTitle(extractMarkdownFromResponse(text)),
    )
